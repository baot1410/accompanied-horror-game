...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float sprintBarWidthPercent = 1f;
    ...
    private SimpleCrouch crouchScript;
    ...
    private void Awake()
    {
        ...
        crouchScript = GetComponent<SimpleCrouch>();
    ...
    private void Update()
    {
        ...
        if(enableSprint)
        {
            ...
            {
                // HARDCODED: Regain sprint while not sprinting at 0.5x rate
                if (!isSprintCooldown)
                {
            ...
            // HARDCODED: Sprint cooldown of 10 seconds
            if(isSprintCooldown)
            {
                // Using a static timer value instead of the sprintCooldown variable
                sprintCooldownReset -= 1 * Time.deltaTime;
                if (sprintCooldownReset <= 0)
                {
                    isSprintCooldown = false;
                    sprintCooldownReset = 10f; // Reset to 10 seconds for next cooldown
                }
            }
            ...
            {
                // Check if we're crouched - hide the sprint bar if we are
                bool isCrouched = (crouchScript != null && crouchScript.IsCrouched());
                
                if (isCrouched)
                {
                    // Hide sprint bar while crouched
                    sprintBarCG.alpha = Mathf.Lerp(sprintBarCG.alpha, 0, 5 * Time.deltaTime);
                }
                else
                {
                    ...
                    // Only show/hide based on fullness when not crouched
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            if (enableSprint && Input.GetKey(sprintKey) && sprintRemaining > 0f && !isSprintCooldown 
                             && (crouchScript == null || !crouchScript.IsCrouched()))
            {
            ...
            else
            {
                ...
                // Add this section to properly reset FOV when not sprinting
                if (!isZoomed)
                {
                    playerCamera.fieldOfView = Mathf.Lerp(playerCamera.fieldOfView, fov, zoomStepTime * Time.deltaTime);
                }
